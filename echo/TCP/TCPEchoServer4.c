...
int main(int argc, char *argv[])
{
  ...
  servAddr.sin_port = htons(servPort);          // Porta local, transformando do formato de host para o formato de rede;

  // Associa ao endereço local
  if (bind(servSock, (struct sockaddr *)&servAddr, sizeof(servAddr)) < 0)
    DieWithSystemMessage("bind() falhou");

  // Marca o socket que irá escutar as requisições de conexão, MAXPENDING determina a quantidade connects simultaneamente.
